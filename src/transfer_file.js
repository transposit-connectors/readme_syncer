({ app_name, from_app, dry_run }) => {
  ...
  var app_blob = api.run("this.find_blob_object", {
    owner: app.owner,
    path: app.path,
    repo: app.repo,
    branch: app.branch,
  })[0];
  var docs_blob = api.run("this.find_blob_object", {
    owner: docs.owner,
    path: docs.path,
    repo: docs.repo,
    branch: docs.branch,
  ...
  var fm_content = api.run("sample_app_helper.generate_frontmatter_for_app", {
    sample_app_url: `https://console.transposit.com/t/transposit-sample/${app_name}`,
  })[0].frontmatter;
  var fm = docs_blob
    ? api.run("front_matter_parser.parse", {
        "$body.content": docs_blob.content,
      })[0]
